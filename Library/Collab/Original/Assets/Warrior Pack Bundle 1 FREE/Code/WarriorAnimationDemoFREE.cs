...
public class WarriorAnimationDemoFREE : MonoBehaviour 
{
	...
	void Update()
	{
		...
		if(stunned == true){
			...
			StartCoroutine (COStunPause(3.0f));
			stunned = false;
			animation.enabled = true;
		}
	}
	void LaunchAttack(Collider other){
		Collider[] cols = Physics.OverlapBox(col.bounds.center,col.bounds.extends,col.transform.rotation,LayerMask.GetMask("Hitbox"));
		foreach(Collider c in cols){
			if(other.gameObject.tag == "player"){
				other.gameObject.isStunned();
		}
		}
